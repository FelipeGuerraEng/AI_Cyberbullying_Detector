...
vectorizer = load('vectorizador.pkl')
...
def analizar():
    ...
    df['ciberacoso'] = model.predict(X)
    df['ciberacoso'] = df['ciberacoso'].map({0: 'No', 1: 'Sí'})
...
dash_app.layout = html.Div([
   ...
   html.Div(
    ...
    dash_table.DataTable(sort_action='native', filter_action='native', column_selectable='single',
        ...
        page_action='native',
        page_size=10,
        columns=[{"name": "Posible víctima" if i == "autor" else i.replace("_", " ").capitalize(), "id": i} for i in
